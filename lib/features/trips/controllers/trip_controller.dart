...
//       {'name': 'Vehicle Return', 'icon': Icons.home},
//     ];
//
//     customStages.clear();
...
//       if (tripId == null) {
//         showErrorMessage('Error', 'Trip not found');
//         return;
//       }
//
//       print('tripId: $tripId');
...
//     try {
//       if (index < 0 || index >= stages.length) {
//         showErrorMessage("Invalid Action", "Invalid stage index");
//         return;
//       }
//
//       final snapshot = await _fireStore
...
//   }
//
//   Future<void> markTripStageWithCustomTime(
//       int stageIndex,
//       DateTime customDateTime,
//       DateTime tripCreatedAt
//       ) async {
//     try {
//       final snapshot = await _fireStore
//           .collection('trips')
//           .where('created_at', isEqualTo: Timestamp.fromDate(tripCreatedAt))
//           .limit(1)
//           .get();
//
//       if (snapshot.docs.isEmpty) {
//         showErrorMessage("Trip Not Found", "No trip found with the given timestamp");
//         return;
//       }
//
//       final tripDoc = snapshot.docs.first;
//       final tripId = tripDoc.id;
//       final stagesData = List<Map<String, dynamic>>.from(tripDoc.data()['stages'] ?? []);
//
...
//       }
//
//       // Update the specific stage with custom timestamp
//       stagesData[stageIndex]['is_completed'] = true;
//       stagesData[stageIndex]['completed_at'] = Timestamp.fromDate(customDateTime);
//
//       // Update trip status to current stage name
//       final currentStageName = stagesData[stageIndex]['name'] ?? '';
//
//       await _fireStore.collection('trips').doc(tripId).update({
//         'stages': stagesData,
//         'trip_status': currentStageName,
...
//       });
//
//       _showSuccessMessage("Stage Completed", "Stage marked as completed successfully");
//     } catch (e) {
//       _handleError('Failed to mark stage as completed', e);
//     }
//   }
...
//     } else {
//       showErrorMessage("Incomplete Trip", "Please complete all stages first");
//     }
//   }
//
...
//     showErrorMessage(title, "An error occurred. Please try again.");
//   }
//
...
class TripController extends GetxController {
  ...
  Future<void> fetchDriversAndVehicles() async {
    try {
      ...
      allTridentAuthorizers.value = futures[6]
          .docs
          .where((doc) => doc.data()['role'] == 'Trip Manager')
          .map((doc) => doc.data()['mobile'] as String ?? '')
          .toList();
  ...
  Future<void> approveTrip() async {
    try {
      ...
      final approvalData = {'in_review': false, 'trip_status': 'Open'};
  ...
  // ================== NEW ENHANCED STAGE COMPLETION METHODS ==================

  /// New method to handle stage completion with custom timestamp
  Future<void> markTripStageWithCustomTime(
      int stageIndex, DateTime customDateTime, DateTime tripCreatedAt) async {
    try {
      final snapshot = await _fireStore
          .collection('trips')
          .where('created_at', isEqualTo: Timestamp.fromDate(tripCreatedAt))
      ...
      // Update the specific stage with custom timestamp
      stagesData[stageIndex]['is_completed'] = true;
      stagesData[stageIndex]['completed_at'] =
      ...
      _showSuccessMessage(
          "Stage Completed", "Stage marked as completed successfully");
    } catch (e) {
      _handleError('Failed to mark stage as completed', e);
    }
  }

  /// Method to get stage completion times for a specific trip
  Future<List<Map<String, dynamic>>> getStageCompletionTimes(
      DateTime tripCreatedAt) async {
    try {
      final snapshot = await _fireStore
          .collection('trips')
          .where('created_at', isEqualTo: Timestamp.fromDate(tripCreatedAt))
      ...
      _handleError('Failed to get stage completion times', e);
      return [];
    }
  }
  ...
  var selectedStatusFilter = 'all'.obs;
  var selectedDriverFilter = 'all'.obs;
  var selectedSortBy = 'date_newest'
      .obs; // date_newest, date_oldest, destination_az, destination_za, driver_az, status
  ...
  List<dynamic> _applyDateFilter(List<dynamic> trips) {
    ...
    return trips.where((trip) {
      ...
      return tripDate.isAfter(startDate) ||
          tripDate.isAtSameMomentAs(startDate);
    }).toList();
  }
  ...
  // Get filtered trips for specific tab status with user exclusion for review
  List<dynamic> getFilteredTripsForStatus(
      List<dynamic> allTrips, String tabStatus,
    ...
    if (tabStatus == 'all') {
      ...
      // For in_review, exclude trips created by current user
      statusFilteredTrips = allTrips.where((trip) {
        final status = (trip.status?.toString().toLowerCase() ?? '');
        final isInReview = status.contains('in_review');

        // If currentUserMobile is provided, exclude user's own trips
        if (currentUserMobile != null && isInReview) {
          final tripCreatedBy = trip.createdBy?.toString() ?? '';
          final normalizedCurrentUser = _normalizeMobile(currentUserMobile);
          final normalizedTripCreator = _normalizeMobile(tripCreatedBy);
          return normalizedCurrentUser != normalizedTripCreator;
        }
      ...
      statusFilteredTrips = allTrips.where((trip) {
        ...
        return status.contains('completed') ||
            status.contains('vehicle return');
      }).toList();
    } else {
      // For specific stages like 'Vehicle Dock', 'Vehicle Loading', etc.
      statusFilteredTrips = allTrips.where((trip) {
  ...
  // Method to check if current user can edit a trip (not created by them)
  bool canEditTrip(dynamic trip, String currentUserMobile) {
    final tripCreatedBy = trip.createdBy?.toString() ?? '';
    final normalizedCurrentUser = _normalizeMobile(currentUserMobile);
    final normalizedTripCreator = _normalizeMobile(tripCreatedBy);
    return normalizedCurrentUser != normalizedTripCreator;
