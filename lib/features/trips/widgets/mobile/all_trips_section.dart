...
class TripsManagementScreen extends StatelessWidget {
  ...
  Widget _buildTabContent(DashBoardController controller,
    ...
    return Obx(() {
      ...
      // Special handling for 'in_review' status - use separate data source
      if (status == 'in_review') {
        // Use the dedicated review trips list which excludes current user's trips
        filteredTrips = controller.allReviewTrips ?? <dynamic>[];
      } else {
        // For all other statuses, use the existing logic with created trips
  ...
  void _refreshTrips(DashBoardController controller) async {
    ...
    if (role == 'driver') {
      ...
      // For non-driver roles, always fetch both created trips and review trips
      await Future.wait([
...
class TripCard extends StatelessWidget {
  ...
  void _showTripActionsBottomSheet() {
    ...
    // Check if trip was created by current user
    final isCreatedByCurrentUser =
        !canEdit; // canEdit is false when user created the trip

    // Check if trip is in review status
    final isInReview = trip.status == 'in_review';
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Logic for showing "View Full Details"
            // Show if: (user created trip AND trip is not in review) OR (user didn't create trip)
            if ((!isCreatedByCurrentUser) ||
                (isCreatedByCurrentUser && !isInReview))
              _buildActionTile(
                'View Full Details',
            ...
            // Logic for showing "Edit Trip"
            // Show only if: user didn't create the trip AND user is not a driver
            if (!isCreatedByCurrentUser && userRole != 'driver')
              _buildActionTile(
                'Edit Trip',
                'Modify trip details',
                Icons.edit_outlined,
            ...
            // Show message if user created trip and it's in review
            if (isCreatedByCurrentUser && isInReview)
              Container(
                padding: EdgeInsets.all(16.w),
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'This trip is currently under review and cannot be viewed or edited.',
                        style: TextStyle(
                          fontSize: 12.sp,
                          color: const Color(0xFFF59E0B),
                          fontWeight: FontWeight.w500,
